...
def get_address() -> dict:
    ...
    addrParam = ['street', 'landmark', 'area', 'city', 'state', 'country', 'pincode']
    try:
        with open(full_path('data.csv'), 'r', newline='', encoding='utf-8') as file:
            addrFile = list(csv.reader(file))
        for i in range(5, 12):
            allAddrs = [row[i] for row in addrFile if len(row) > i and row[i].strip()]
            if not allAddrs:
                full_addr.append("Unknown")  # або raise ValueError(f"No data in column {i}")
            else:
                full_addr.append(choice(allAddrs))
        return dict(zip(addrParam, full_addr))
    except FileNotFoundError:
        raise FileNotFoundError("File 'data.csv' not found. Make sure the path is correct.")
