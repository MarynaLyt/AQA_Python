...
filtered_cars = []
for brand, description in car_data.items():
    year, engine_volume, price = description [1], description [2], description [4]
    if year >= search_criteria[0] and engine_volume >= search_criteria[1] and price <= search_criteria[2]:
        filtered_cars.append((brand, description))
sorted_filtered_cars = sorted(filtered_cars[:5], key=lambda x: x[1][4])
print(sorted_filtered_cars)